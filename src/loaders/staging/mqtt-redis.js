...
mqttClient.on('message', function (topic, message) {
    ...
    readCommandAsync(payload.messageId)
          ...
          if (commandWaiting.hasOwnProperty('source')) {
            ...
            if (commandSource == 'Alexa') {
              if (commandWaiting.hasOwnProperty('response')) {
                logger.log(
                  'warn',
                  '[Alexa API] Failed Alexa MQTT Command for user: ' +
                    username +
                    ', response: ' +
                    +JSON.stringify(commandWaiting.response)
                );
                ...
                logger.log(
                  'warn',
                  '[Alexa API] Failed Alexa MQTT Command for user: ' + username
                );
            ...
            if (commandSource == 'Google') {
              ...
              logger.log(
                'warn',
                '[GHome API] Failed Google Home MQTT command for user: ' +
                  username +
                  ', response: ' +
                  JSON.stringify(commandWaiting.response)
              );
              try {
                ...
                logger.log(
                  'warn',
                  '[GHome API] Error sending failed command response for user: ' +
                    username +
                    ', error: ' +
                    e
                );
          ...
          if (commandSource == 'Google') {
            ...
            if (
              ...
              if (
                ...
                commandWaiting.response.payload.commands[0].ids.push(
                  endpointId
                );
              }
              ...
              setAsync(payload.messageId + endpointId, commandWaiting).then(
                (result) => {
                  for (x = 0; x < arrCommandDevices.length; x++) {
                    ...
                    readCommandAsync(payload.messageId).then(
                      (additionalCommand) => {
                        if (
                          ...
                          if (additionalCommand.acknowledged == true) {
                            ...
                            logger.log(
                              'debug',
                              '[GHome API] Merged *acknowledged* multi-device command response: ' +
                                username +
                                ', ***updated*** response: ' +
                                JSON.stringify(commandWaiting.response)
                            );
                          ...
                          // This additional command is yet to be acknowledged via MQTT response from Node-RED
                            ...
                            logger.log(
                              'debug',
                              '[GHome API] Google Home multi-device command *not* acknowledged: ' +
                                username +
            ...
            ///////////////////////////////////////////////////////////////////////////
              ...
              try {
                ...
                logger.log(
                  'debug',
                  '[GHome API] Successful Google Home MQTT command for user: ' +
                    username +
                    ', response: ' +
                    JSON.stringify(commandWaiting.response)
                );
      ...
      .then((commandWaiting) => {
        ...
        if (commandSource == 'Google' && sendResponse == true) {
          logger.log(
            'debug',
            '[GHome API] Successful Google Home multi-device command for user: ' +
              username +
              ', response: ' +
              JSON.stringify(commandWaiting.response)
          );
  ...
  if (topic.startsWith('state/')) {
    ...
    logger.log(
      'info',
      '[State API] Acknowledged MQTT state message for user: ' +
        username +
        ', message: ' +
        message
    );
    ...
    logger.log(
      'debug',
      '[MQTT] Unhandled MQTT via on message event handler: ' + topic + message
    );
  }
});
    ...
    for (key in keys) {
      ...
      readCommandAsync(key)
        ...
        .then((waiting) => {
          logger.log(
            'debug',
            '[MQTT] Unacknowledged Alexa command message for user: ' +
              waiting.user +
              ', message: ' +
              waiting
          );
        ...
        .then((waiting) => {
          logger.log(
            'debug',
            '[MQTT] Unacknowledged GHome command message for user: ' +
              waiting.user +
              ', message: ' +
              waiting
          );
            ...
            if (waiting.hasOwnProperty('source')) {
              ...
              if (commandSource == 'Google') {
                ...
                if (
                  ...
                  logger.log(
                    'debug',
                    '[MQTT] GHome Multi-device command waiting'
                  );
                  ...
                  for (x = 0; x < arrCommandDevices.length; x++) {
                    readAsync(waiting.requestId + arrCommandDevices[x])
                      .then((additionalCommand) => {
                        if (
                          ...
                          if (response == undefined) {
                            ...
                            response.payload.commands[0].ids.push(
                              arrCommandDevices[x]
                            );
                          }
                        ...
                        var diffAdditionalCommand =
                          now - additionalCommand.timestamp;
                        if (
                          diffAdditionalCommand > maxTime &&
                          response !== undefined
                        ) {
                          delAsync(
                            additionalCommand.requestId + arrCommandDevices[x]
                      ...
                      .catch((err) => {
                        logger.log(
                          'error',
                          '[MQTT] Error reading GHome additional command data from Redis, error message: ' +
                            err
                        );
                      });
                  }
        ...
        .then((waiting) => {
          ...
          if (commandSource == 'Google' && response !== undefined) {
            logger.log(
              'debug',
              '[MQTT] Partial success GHome multi-device command, response: ' +
            ...
            try {
              ...
              logger.log(
                'warn',
                '[MQTT] Send GHome multi-command response error: ' + e
              );
              delAsync(key);
            }
          }
          // No acknowledged commands, so send timeout
            ...
            try {
              ...
              logger.log(
                'debug',
                '[MQTT] GHome command timed-out, unable to send result, error: ' +
                  e
              );
              delAsync(key);
            }
