...
const updateUserServices =
  require('../services/func-services').updateUserServices;
const queryDeviceStateAsync =
  require('../services/func-alexa').queryDeviceStateAsync;
const replaceCapability =
  require('../services/func-alexa').replaceCapabilityAsync;
const saveGrantAsync = require('../services/func-alexa').saveGrantAsync;
const requestAccessTokenAsync =
  require('../services/func-alexa').requestAccessTokenAsync;
const validateCommandAsync =
  require('../services/func-alexa').validateCommandAsync;
const buildCommandResponseAsync =
  require('../services/func-alexa').buildCommandResponseAsync;
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      logger.log(
        'debug',
        '[Alexa Discovery] Alexa Discovery response for user: ' +
          req.user.username +
          ', response: ' +
          JSON.stringify(devs)
      );
      ...
      logger.log(
        'error',
        '[Alexa Discovery] Error getting device data for: ' +
          req.user.username +
          ', error: ' +
          e.stack
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      logger.log(
        'error',
        '[Alexa State API] Error getting state for endpointId: ' +
          req.params.dev_id +
          ', error: ' +
          e.stack
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.log(
        'debug',
        '[Alexa API] Received command for user: ' +
          req.user.username +
          ', command: ' +
      ...
      logger.log(
        'debug',
        '[Alexa Command] Received command API request for user: ' +
          req.user.username +
          ' command: ' +
          message
      );
      ...
      logger.log(
        'info',
        '[Alexa Command] Published MQTT command for user: ' +
          req.user.username +
          ' topic: ' +
          topic
      );
      ...
      logger.log(
        'error',
        '[Alexa Command] Failed to execute command for user: ' +
          req.user.username +
          ', error: ' +
          e.stack
      );
...
router.post(
  ...
  async (req, res) => {
    ...
    if (req.body.directive.payload.grant.type == 'OAuth2.AuthorizationCode') {
      logger.log(
        'info',
        '[AlexaAuth] Received authorisation request, body: ' +
          JSON.stringify(req.body)
      );
      ...
      if (grant != undefined) {
        ...
        logger.log(
          'debug',
          '[AlexaAuth] Sent authorisation response for user :' +
            req.user.username +
            ',  body: ' +
        ...
        if (accessToken == undefined)
          logger.log(
            'error',
            '[AlexaAuth] Failed to obtain AccessToken for user: ' +
              req.user.username
          );
      } else {
        logger.log(
          'error',
          '[AlexaAuth] General authorisation failure, sending: ' +
