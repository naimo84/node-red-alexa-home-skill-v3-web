...
Mailer.prototype.send = function send(to, from, subject, text, html, callback) {
  ...
  transporter.sendMail(message, function (error, info) {
    if (error) {
      logger.log(
        'error',
        '[Send Email] Error sending email, subject: ' +
          message.subject +
          ', to email address: ' +
          message.to +
          ', error: ' +
...
Mailer.prototype.buildLostPasswordBody = function buildLostBody(
  uuid,
  userid,
  fqdn
) {
  var body = ejs.render(lostPasswordTxtTemplate, {
    uuid: uuid,
    username: userid,
    fqdn: fqdn,
    brand: process.env.BRAND,
  });
  var htmlBody = ejs.render(lostPasswordHTMLTemplate, {
    uuid: uuid,
    username: userid,
    fqdn: fqdn,
    brand: process.env.BRAND,
...
Mailer.prototype.buildVerifyEmail = function buildVerifyEmail(
  token,
  userid,
  fqdn
) {
  var body = ejs.render(verifyEmailTxtTemplate, {
    token: token,
    username: userid,
    fqdn: fqdn,
    brand: process.env.BRAND,
  });
  var htmlBody = ejs.render(verifyEmailHTMLTemplate, {
    token: token,
    username: userid,
    fqdn: fqdn,
    brand: process.env.BRAND,
