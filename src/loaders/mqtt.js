...
mqttClient.on('connect', function () {
  logger.log(
    'info',
    "[MQTT] MQTT connected, subscribing to 'response/#' and 'state/#'"
  );
...
mqttClient.on('message', function (topic, message) {
  ...
  var commandWaiting =
    ongoingCommands[payload.messageId] ||
    ongoingCommands[payload.messageId + endpointId];
    ...
    if (payload.success) {
      ...
      if (Array.isArray(arrCommandDevices) && arrCommandDevices.length !== 0) {
            ...
            if (
              ...
              if (additionalCommand.acknowledged == true) {
                ...
                delete ongoingCommands[
                  additionalCommand.requestId + arrCommandDevices[x]
                ];
                logger.log(
                  'debug',
                  '[MQTT] Merged *acknowledged* GHome multi-device command response: ' +
                    username +
                    ', ***updated*** response: ' +
                    JSON.stringify(commandWaiting.response)
                );
              }
              // This additional command is yet to be acknowledged via MQTT response from Node-RED
                ...
                logger.log(
                  'debug',
                  '[MQTT] GHome multi-device command *not* acknowledged: ' +
                    username +
        ...
        if (sendResponse == true) {
          logger.log(
            'debug',
            '[MQTT] Successful GHome multi-device command for user: ' +
              username +
              ', response: ' +
              JSON.stringify(commandWaiting.response)
          );
      ...
      ///////////////////////////////////////////////////////////////////////////
        ...
        try {
          ...
          logger.log(
            'debug',
            '[MQTT] Successful GHome MQTT command for user: ' +
              username +
              ', response: ' +
              JSON.stringify(commandWaiting.response)
          );
      ...
      logger.log(
        'debug',
        '[MQTT] GHome MQTT response message is failure for topic: ' + topic
      );
      ...
      if (commandWaiting.hasOwnProperty('source')) {
        ...
        if (commandSource == 'Google') {
          ...
          logger.log(
            'warn',
            '[MQTT] Failed GHome MQTT command for user: ' +
              username +
              ', response: ' +
              JSON.stringify(commandWaiting.response)
          );
          try {
            ...
            logger.log(
              'warn',
              '[MQTT] Error sending GHome failed command response for user: ' +
                username +
                ', error: ' +
                e
            );
  ...
  /// Start Alexa Response
    ...
    logger.log(
      'info',
      '[MQTT] Acknowledged Alexa MQTT response message for topic: ' + topic
    );
    if (payload.success) {
      ...
      if (commandWaiting.hasOwnProperty('response')) {
        logger.log(
          'debug',
          '[MQTT] Successful Alexa MQTT command for user: ' +
            username +
            ', response: ' +
            JSON.stringify(commandWaiting.response)
        );
        ...
        logger.log(
          'debug',
          '[MQTT] Successful Alexa MQTT command for user: ' + username
        );
      ...
      if (commandWaiting.hasOwnProperty('source')) {
        ...
        if (commandSource == 'Alexa') {
          if (commandWaiting.hasOwnProperty('response')) {
            logger.log(
              'warn',
              '[MQTT] Failed Alexa MQTT Command for user: ' +
                username +
                ', response: ' +
                +JSON.stringify(commandWaiting.response)
            );
            ...
            logger.log(
              'warn',
              '[MQTT] Failed Alexa MQTT Command for user: ' + username
            );
  ...
  /// Start State Handler
    ...
    logger.log(
      'info',
      '[MQTT] Acknowledged MQTT State message for user: ' +
        username +
        ', message: ' +
        message
    );
    ...
    updateDeviceState(username, endpointId, payload) //arrTopic[1] is username, arrTopic[2] is endpointId
      .then((result) => {
        if (result == true) {
          logger.log(
            'verbose',
            '[MQTT] Successfully updated state for user: ' +
              username +
              ', endpointId: ' +
              endpointId
          );
          ...
          logger.log(
            'warn',
            '[MQTT] Failed to updated state for user: ' +
              username +
              ', endpointId: ' +
              endpointId
          );
      ...
      .catch((e) => {
        logger.log(
          'error',
          '[MQTT] Error trying to update state for user: ' +
            username +
            ', endpointId: ' +
            endpointId +
            ', error' +
    ...
    logger.log(
      'debug',
      '[MQTT] Unhandled MQTT message event: ' + topic + message
    );
  }
});
    ...
    if (waiting && waiting.source == 'Alexa') {
      ...
      if (diff > maxTime) {
        try {
          ...
          logger.log(
            'warn',
            '[MQTT] Sent Alexa time-out response for user: ' +
              waiting.user +
              ', message: ' +
              keys[key]
          );
          ...
          logger.log(
            'error',
            '[MQTT] Error sending Alexa timeout response, error: ' + e
          );
        }
      ...
      if (diff > maxTime) {
        ...
        if (
          Array.isArray(arrCommandDevices) &&
          arrCommandDevices.length !== 0
        ) {
          ...
          if (waiting.acknowledged == true) {
            response = waiting.response;
          }
          // Check for linked commands with same referenceId in ongoingCommands
          for (x = 0; x < arrCommandDevices.length; x++) {
            var additionalCommand =
              ongoingCommands[waiting.requestId + arrCommandDevices[x]];
            if (additionalCommand) {
              if (
                additionalCommand.hasOwnProperty('acknowledged') &&
                additionalCommand.acknowledged == true
              ) {
                ...
                if (response == undefined) {
                  ...
                  response.payload.commands[0].ids.includes(
                    arrCommandDevices[x]
                  ) == false
                ) {
                  response.payload.commands[0].ids.push(arrCommandDevices[x]);
          ...
          if (response !== undefined) {
            logger.log(
              'debug',
              '[MQTT] Successful GHome multi-device command, response: ' +
                JSON.stringify(waiting.response)
            );
            try {
              // Multi-devices this generates an error as res is sent after first device
              ...
              logger.log(
                'warn',
                '[MQTT] Send multi-command GHome response error: ' + e
              );
              delete ongoingCommands[keys[key]];
            }
          }
          // No acknowledged commands, so send timeout
            ...
            try {
              ...
              logger.log(
                'debug',
                '[MQTT] GHome multi-device command timed-out, unable to send result, error: ' +
                  e
              );
              delete ongoingCommands[keys[key]];
            }
        ...
        ///////////////////////////////////////////////////////////////////////////
          ...
          try {
            ...
            logger.log(
              'debug',
              '[MQTT] GHome single-device command timed-out, unable to send result, error: ' +
                e
            );
            delete ongoingCommands[keys[key]];
          }
...
function notifyUser(severity, username, endpointId, message) {
  ...
  try {
    logger.log(
      'debug',
      '[MQTT] Publishing MQTT alert, topic: ' +
        topic +
        ', alert: ' +
        alertString
    );
    ...
    logger.log(
      'warn',
      '[MQTT] Published MQTT alert for user: ' +
        username +
        ' endpointId: ' +
        endpointId +
        ' message: ' +
    ...
    logger.log(
      'error',
      '[MQTT] Failed to publish MQTT alert, error: ' + err.stack
    );
  }
}
