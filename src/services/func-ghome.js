...
const validateCommandAsync = async (command, commandDevice, dbDevice, req) => {
  try {
    ...
    if (
      ...
      var hasTemperatureMin = getSafe(
        () => dbDevice.attributes.temperatureRange.temperatureMin
      );
    ...
    if (
      ...
      var hasTemperatureMinK = getSafe(
        () => dbDevice.attributes.colorTemperatureRange.temperatureMinK
      );
