...
const setupServiceAccount =
  require('./services/func-services').setupServiceAccount;
///////////////////////////////////////////////////////////////////////////
// Variables
///////////////////////////////////////////////////////////////////////////
...
if (cookieSecret == 'ihytsrf334') {
  logger.log(
    'warn',
    '[App] Using default Cookie Secret, please supply new secret using COOKIE_SECRET environment variable'
  );
} else {
  logger.log('info', '[App] Using user-defined cookie secret');
}
///////////////////////////////////////////////////////////////////////////
// Main
///////////////////////////////////////////////////////////////////////////
...
if (dotenv.error) {
  logger.log(
    'error',
    '[Core] dotenv parsing failed, please ensure you have mapped .env file to /usr/src/app/.env, an example file is provided, see .env.template for more information'
  );
  throw dotenv.error;
}
...
if (
  ...
  logger.log(
    'error',
    '[Core] You MUST supply MONGO_USER, MONGO_PASSWORD, MQTT_USER, MQTT_PASSWORD and MQTT_PORT environment variables'
  );
  process.exit(1);
}
...
if (!(process.env.MONGO_HOST && process.env.MQTT_URL)) {
  logger.log(
    'warn',
    "[Core] Using 'mongodb' for Mongodb and 'mosquitto' for MQTT service endpoints, no MONGO_HOST/ MQTT_URL environment variable supplied"
  );
}
...
if (
  ...
  logger.log(
    'warn',
    '[Core] No MAIL_SERVER/ MAIL_USER/ MAIL_PASSWORD environment variable supplied. System generated emails will generate errors'
  );
}
...
passport.use(
  new LocalStrategy((username, password, cb) => {
    authenticate(username, password, (err, user, error) => {
      logger.log(
        'debug',
        '[Auth] Passport Local Strategy, authentication called for user: ' +
          username
      );
...
passport.use(
  new BasicStrategy((username, password, cb) => {
    authenticate(username, password, (err, user, error) => {
      logger.log(
        'debug',
        '[Auth] Passport Basic Strategy, authentication called for user: ' +
          username
      );
      // An error ocurred, do not authenticate
      if (err) {
...
passport.use(
  new PassportOAuthBearer(function (token, done) {
    oauthModels.AccessToken.findOne({ token: token })
      ...
      .exec(function (error, token) {
        if (!error && token) {
          logger.log(
            'debug',
            '[OAuth] Returned OAuth Token: ' + JSON.stringify(token)
          );
          ...
          if (
            ...
            logger.log(
              'verbose',
              '[OAuth] OAuth Token success for user: ' +
                token.user.username +
                ', token: ' +
          ...
          // Found OAuth token, however token not active
            ...
            logger.log(
              'warn',
              '[OAuth] OAuth Token failure, token not active: ' +
                JSON.stringify(token)
            );
            done(null, false);
          }
          // Found OAuth token, however token has no grant
            ...
            logger.log(
              'warn',
              '[OAuth] OAuth Token failure, missing grant token: ' +
                JSON.stringify(token)
            );
            done(null, false);
          }
          // Found OAuth token, however token grant not active
            ...
            logger.log(
              'warn',
              '[OAuth] OAuth Token failure, grant token not active: ' +
                JSON.stringify(token)
            );
            done(null, false);
          }
          // Found OAuth token, however token user missing (should never get here!)
            ...
            logger.log(
              'warn',
              '[OAuth] OAuth Token failure, user population failed: ' +
                JSON.stringify(token)
            );
            done(null, false);
          }
          // Found OAuth token, however user is not active/ enabled
            ...
            logger.log(
              'warn',
              '[OAuth] OAuth Token failure, user: ' +
                token.user.username +
        ...
        // No OAuth token found
          ...
          logger.log(
            'warn',
            '[OAuth] OAuth Token failure, token not found for user!'
          );
          done(null, false);
        }
        // An Error occurred in trying to find OAuth token
          ...
          logger.log(
            'error',
            '[OAuth] OAuth Token lookup failed, error: ' + error
          );
          done(error);
        }
      });
...
const createServer = async () => {
  try {
    ...
    var boolSetupServiceAccount = await setupServiceAccount(
      mqtt_user,
      mqtt_password
    );
    ...
    if (app.get('env') === 'production') {
      logger.log(
        'info',
        '[App] Production environment detected enabling trust proxy/ secure cookies'
      );
    ...
    app.use(
      '/static',
      express.static(path.join(__dirname, '/interfaces/static'))
    );
    app.use(
      '/static/octicons',
      express.static('node_modules/@primer/octicons/build'),
      express.static('node_modules/@primer/octicons/build/svg')
    ); // Octicons router
    // Setup robots.txt
    app.use(
      robots({
        ...
        Allow: '/',
        CrawlDelay: '5',
        Sitemap: 'https://' + process.env.WEB_HOSTNAME + '/sitemap.xml',
      })
    );
    ...
    app.get('/sitemap.xml', function (req, res) {
      ...
      try {
        const smStream = new SitemapStream({
          hostname: 'https://' + process.env.WEB_HOSTNAME + '/',
        });
    ...
    app.use(function (err, req, res, next) {
      if (err.status == 404) {
        logger.log(
          'warn',
          '[App] Not found: ' + err.status + ', path: ' + req.path
        );
      } else {
        logger.log(
          'error',
          '[App] Fall-back error handler, status code: ' +
            err.status +
            ', message: ' +
            err.message
        );
    ...
    logger.log(
      'error',
      '[App] Create Server catch error handler, error: ' + e.stack
    );
  }
};
