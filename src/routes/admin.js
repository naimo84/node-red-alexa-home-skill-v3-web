...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      logger.log(
        'error',
        '[Admin Services] Error rendering page, error: ' + e.stack
      );
...
router.get('/users', defaultLimiter, ensureAuthenticated, async (req, res) => {
  try {
    ...
    logger.log(
      'error',
      '[Admin Users] Error rendering page, error: ' + e.stack
    );
    res.status(500).send('Error rendering page!');
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.superuser === true) {
        ...
        logger.log(
          'debug',
          '[Reset Topics] Reset MQTT topics for user: ' +
            account.username +
            ', to: ' +
      ...
      logger.log(
        'error',
        '[Reset Topics] Failed to reset topics for user, error: ' + e.stack
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.log(
        'error',
        '[Admin Users] Error disabling/ enabling user, error: ' + e.stack
      );
      return res.status(400).send('Error updating account state!');
    }
  }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      logger.log(
        'error',
        '[Admin Devices] Error rendering page, error: ' + e.stack
      );
      res.status(500).send('Error rendering page!');
    }
  }
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      logger.log(
        'error',
        '[Admin Services] Error saving new service, error: ' + e.stack
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.log(
        'error',
        '[Admin Services] Error editing service, error: ' + e.stack
      );
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      logger.log(
        'error',
        '[Admin Services] Error deleting service, error: ' + e.stack
      );
...
const toggleUser = async (id, enabled) => {
  try {
    ...
    if (enabled == true && account.username != mqtt_user) {
      ...
      // Set MQTT password
      account.mqttPass =
        'PBKDF2$sha256$901$' + account.salt + '$' + account.hash;
      logger.log(
        'verbose',
        '[Admin] Enabling User Account: ' + account.username
      );
      ...
      logger.log(
        'verbose',
        '[Admin] Disabling User Account: ' + account.username
      );
    } else {
      logger.log(
        'error',
        '[Admin] toggleUser invalid state requested: ' + enabled
      );
      return false;
    }
    // Save Account
    ...
    logger.log(
      'verbose',
      "[Admin] Account saved following 'active' element change: " +
        account.username
    );
    return true;
  } catch (e) {
    logger.log(
      'error',
      "[Admin] Unable to change user 'active' element, error: " + e
    );
    return false;
  }
};
