...
const gHomeReplaceCapability =
  require('../services/func-ghome').gHomeReplaceCapability;
const gHomeReplaceType = require('../services/func-ghome').gHomeReplaceType;
const validateCommandAsync =
  require('../services/func-ghome').validateCommandAsync;
    ...
    switch (intent) {
      ...
      case 'action.devices.SYNC':
        try {
          logger.log(
            'verbose',
            '[GHome Sync API] Running device discovery for user: ' +
              req.user.username
          );
          ...
          logger.log(
            'debug',
            '[GHome Sync API] User: ' + JSON.stringify(user[0])
          );
          // Build Device Array
          var devs = [];
          ...
          for (let device of devices) {
            ...
            if (
              device.capabilities.indexOf('ColorTemperatureController') > -1
            ) {
              dev.attributes.colorTemperatureRange.temperatureMinK = parseInt(
                dev.attributes.colorTemperatureRange.temperatureMinK
              );
              dev.attributes.colorTemperatureRange.temperatureMaxK = parseInt(
                dev.attributes.colorTemperatureRange.temperatureMaxK
              );
            }
            // FanSpeed, map 1:1 with RangeController 1-10
            if (
              device.capabilities.indexOf('RangeController') > -1 &&
              (dev.type.indexOf('action.devices.types.FAN') > -1 ||
                dev.type.indexOf('action.devices.types.THERMOSTAT') > -1)
            ) {
            ...
            if (
              dev.traits.indexOf('action.devices.traits.TemperatureSetting') >
              -1
            ) {
              ...
              // }
              dev.attributes.availableThermostatModes =
                dev.attributes.thermostatModes.join().toLowerCase(); // Make string, not array
              dev.attributes.thermostatTemperatureUnit =
                dev.attributes.temperatureScale.substring(0, 1); // >> Need to make this upper F or C, so trim
          ...
          logger.log(
            'verbose',
            '[GHome Sync API] Discovery Response for user: ' +
              req.user.username +
              ', response: ' +
      ...
      case 'action.devices.EXECUTE':
        try {
          ...
          logger.log(
            'verbose',
            '[GHome Exec API] Execute command(s) for user: ' +
              req.user.username +
              ', command: ' +
              JSON.stringify(req.body.inputs[0].payload.commands)
          );
          // Create array of commands
          var arrCommands = req.body.inputs[0].payload.commands;
          logger.log(
            'debug',
            '[GHome Exec API] # of commands in request: ' + arrCommands.length
          );
          ...
          for (let command of arrCommands) {
            ...
            logger.log(
              'debug',
              '[GHome Exec API] # of endpoints in command request: ' +
            ...
            for (let commandDevice of command.devices) {
              logger.log(
                'debug',
                '[GHome Exec API] Executing command for device id: ' +
                  commandDevice.id
              );
              ...
              if (dbDevice == undefined) {
                logger.log(
                  'debug',
                  '[GHome Exec API] Failed to match device against devicesJSON'
                );
              } else {
                logger.log(
                  'debug',
                  '[GHome Exec API] Executing command against device:' +
                    JSON.stringify(dbDevice)
                );
              }
              ...
              logger.log(
                'debug',
                '[GHome Exec API] Command to be executed against endpointId:' +
                  commandDevice.id
              );
              // Define MQTT Topic
              var topic =
                'command/' + req.user.username + '/' + commandDevice.id;
              // Define MQTT Message
              ...
              logger.log(
                'verbose',
                '[GHome Exec API] Published MQTT command for user: ' +
                  req.user.username +
                  ' topic: ' +
                  topic
              );
              ...
              for (let device of command.devices) {
                ...
                if (device.id != undefined && device.id != commandDevice.id) {
                  ...
                  logger.log(
                    'debug',
                    '[GHome Exec API] Added endpointId to multi-device command'
                  );
                }
              }
      ...
      case 'action.devices.QUERY':
        try {
          logger.log(
            'verbose',
            '[GHome QUERY API] Running device state query for user: ' +
              req.user.username
          );
          ...
          logger.log(
            'verbose',
            '[GHome QUERY API] QUERY state: ' + JSON.stringify(response)
          );
      ...
      case 'action.devices.DISCONNECT':
        try {
          ...
          logger.log(
            'error',
            '[GHome Disconnect API] Failed to delete GrantCodes, RefreshToken and AccessTokens for user account: ' +
              userId +
              ', error: ' +
              e.stack
          );
