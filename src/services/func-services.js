...
const updateUserServices = async (username, applicationName) => {
  try {
    ...
    logger.log(
      'verbose',
      '[Services] Added service: ' +
        applicationName +
        ' to activeServices for user:' +
        username
    );
  } catch (e) {
    logger.log(
      'error',
      '[Services] Unable to add service to activeServices, error:' + e.stack
    );
  }
};
  ...
  try {
    ...
    logger.log(
      ...
      '[Services] Removed service: ' +
        applicationName +
        ' from activeServices for user:' +
        username
    );
  } catch (e) {
    logger.log(
      'error',
      '[Services] Unable to remove service from activeServices, error:' +
        e.stack
    );
  }
};
...
const createACL = async (pattern) => {
  try {
    ...
    if (!acl) {
      ...
      logger.log(
        'debug',
        '[Topics] Created command pattern-based MQTT topic: ' +
      ...
      logger.log(
        'debug',
        '[Topics] Skipped create MQTT topic, already exists: ' + pattern
      );
      return acl;
    }
  } catch (e) {
    logger.log(
      'error',
      '[Topics] Unable to save command pattern-based MQTT topic, error: ' +
        e.stack
    );
    return undefined;
  }
...
const setupServiceAccount = async (username, password) => {
  try {
    ...
    if (!account) {
      logger.log(
        'info',
        '[App] Superuser MQTT account not found, creating account: ' + username
      );
    ...
    // Super User set active
      ...
      logger.log(
        'info',
        '[App] Superuser MQTT found but needs updating, modifying account: ' +
          username
      );
      await Account.updateOne(
        { username: account.username },
    ...
    // Super User account found and configured correctly
      ...
      logger.log(
        'info',
        '[App] Superuser MQTT account, ' +
          username +
          ' already exists/ is configured correctly!'
      );
      return true;
    }
  } catch (e) {
    logger.log(
      ...
      '[App] Superuser MQTT account, ' +
        username +
        ' configuration/ creation failed, error: ' +
        e.stack
    );
    return false;
  }
