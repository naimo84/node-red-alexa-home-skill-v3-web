          ...
          OAuth.AccessToken.findOne(
            ...
            function (error, token) {
              if (token) {
                logger.log(
                  'debug',
                  '[OAuth Server] Found valid AccessToken for user: ' +
                    grant.user
                );
                OAuth.RefreshToken.findOne(
                  ...
                  function (error, refreshToken) {
                    if (refreshToken) {
                      logger.log(
                        'debug',
                        '[OAuth Server] Found valid RefreshToken for user: ' +
                          grant.user
                      );
                      ...
                      logger.log(
                        'debug',
                        '[OAuth Server] AccessToken sent expires_in: ' + expires
                      );
                    } else {
                      ...
                      logger.log(
                        'debug',
                        '[OAuth Server] Error, could not find valid RefreshToken for user: ' +
                          grant.user
                      );
                      done(error);
                    }
                ...
                logger.log(
                  'debug',
                  '[OAuth Server] Valid AccessToken not found for user: ' +
                    grant.user
                );
                  ...
                  OAuth.RefreshToken.findOne(
                    ...
                    function (error, refreshToken) {
                      if (refreshToken) {
                        logger.log(
                          'debug',
                          '[OAuth Server] Created AccessToken for user: ' +
                            grant.user +
                            ', expires_in:' +
                            expires
                        );
                        logger.log(
                          'debug',
                          '[OAuth Server] AccessToken:' + JSON.stringify(token)
                        );
                        done(
                          error,
                        ...
                        logger.log(
                          'debug',
                          '[OAuth Server] Created RefreshToken for user: ' +
                            grant.user
                        );
                        logger.log(
                          'debug',
                          '[OAuth Server] refreshToken:' +
      ...
      OAuth.RefreshToken.findOne({ token: token }, function (error, refresh) {
        if (refresh && refresh.application == application.id) {
              ...
              if (
                ...
                newToken.save(function (error) {
                  ...
                  if (!error) {
                    ...
                    logger.log(
                      'debug',
                      '[OAuth Server] AccessToken:' + JSON.stringify(newToken)
                    );
                    ...
                    logger.log(
                      'debug',
                      '[OAuth Server] Error saving token, error:' + error
                    );
                    done(error, false);
                  }
                });
                ...
                if (!grant) {
                  logger.log(
                    'debug',
                    '[OAuth Server] Error, GrantCode not found'
                  );
                }
                if (!grant.active) {
                  logger.log('debug', '[OAuth Server] Error, grant not active');
                }
                if (grant.application != application.id) {
                  logger.log(
                    'debug',
                    '[OAuth Server] Error, grant.application: ' +
                      grant.application +
                      ' does not match application.id:' +
                      application.id
                  );
                }
                if (error) {
                  logger.log(
                    'error',
                    '[OAuth Server] Find GrantCode error:' + error
                  );
                }
                done(error, null);
              }
          ...
          if (!refresh && application != null) {
            logger.log(
              'debug',
              '[OAuth Server] Error, refresh token not found for application:' +
                application.title
            );
          } else if (!application) {
            logger.log(
              'error',
              '[OAuth Server] Error, unable to find refresh token, application is null'
            );
          }
          ...
          if (error) {
            logger.log(
              'error',
              '[OAuth Server] Find RefreshToken error:' + error
            );
          }
          done(error, false);
        }
