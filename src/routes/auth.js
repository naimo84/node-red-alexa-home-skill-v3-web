    ...
    oauthModels.Application.findOne(
      ...
      function (error, application) {
        if (application) {
          logger.log(
            'debug',
            '[Oauth2] Starting Auth for application:' + application.title
          );
          var match = false,
          ...
          for (var i = 0; i < application.domains.length; i++) {
            logger.log(
              'info',
              '[Oauth2] Checking OAuth redirectURI against defined service domain: ' +
            ...
            if (
              ...
              logger.log(
                'info',
                '[Oauth2] Found Service definition associated with redirectURI: ' +
                  redirectURI
              );
              break;
            }
...
router.post(
  ...
  function (req, res, next) {
    if (req.user) {
      logger.log(
        'debug',
        '[OAuth2] User already logged in:' + req.user.username
      );
      next();
    } else {
    ...
    oauthModels.Application.findOne(
      ...
      function (error, application) {
        if (application) {
          ...
          logger.log(
            'debug',
            '[Oauth2] Could not find service definition associated with applicationID: ' +
              appID +
              ' or secret: ' +
              appSecret
          );
